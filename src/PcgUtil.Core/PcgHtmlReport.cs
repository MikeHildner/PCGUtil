...
/// set lists, the program usage report. Output is HTML-escaped with simple print-friendly CSS.
///
/// These are separate documents rather than printable views inside the app, and that is
/// deliberate: the app follows the reader's light/dark theme and its stylesheet carries no
/// print rules, so an in-page report would print dark on a gig-bag printer. A downloaded
/// document never sees the app's theme and always prints light. (The per-slot gig sheet
/// outgrew HTML and is now drawn straight to PDF — see <see cref="GigSheetPdf"/>.)
/// </summary>
