...
builder.Services.AddRazorComponents()
    .AddInteractiveServerComponents(options =>
    {
        ...
        // each retained circuit can pin the editable target PCG (~47 MB) plus TWO read-only
        // Merge sources (~94 MB) plus up to 64 MB of undo patches (PcgEditHistory's cap;
        // realistic sessions hold a few MB), so ~205 MB per circuit worst case. Three
        // retained circuits ≈ ≤ 615 MB — leaving headroom for live circuits and the
        // 150 MB UploadCache.
